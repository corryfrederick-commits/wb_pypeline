{{ config(materialized='table', schema='core', alias='report_order_events', tags=['core', 'core_reports_finance', 'scd2']) }}
    ...
    select
        *,
        ...
        coalesce(
            ...
            nullif(g_number, ''),
            raw_payload_id::text || ':' || record_index::text
        ) as report_order_event_natural_id
    from source
    ...
    select
        *,
        md5(concat_ws(
            ...
            coalesce(cancel_date::text, ''),
            coalesce(is_cancel::text, ''),
            ...
            coalesce(sticker, '')
        )) as report_order_event_row_hash
    from prepared
...
ordered as (
    ...
    select
        *,
        lag(report_order_event_row_hash) over (
            partition by client_id, wb_account_id, report_order_event_natural_id
            order by loaded_at, raw_payload_id, record_index
        ) as previous_report_order_event_row_hash
    from hashed
...
version_starts as (
    ...
    select *
    from ordered
    where previous_report_order_event_row_hash is null
       or previous_report_order_event_row_hash <> report_order_event_row_hash
...
select
    ...
    md5(concat_ws('||', client_id, wb_account_id, 'report_order_event', report_order_event_natural_id)) as report_order_event_key,
    md5(concat_ws('||', client_id, wb_account_id, 'report_order_event_version', report_order_event_natural_id, version_number::text, report_order_event_row_hash)) as report_order_event_version_key,

    report_order_event_natural_id,
    report_order_event_row_hash,
    version_number,
    valid_from,
    valid_to,
    valid_to is null as is_current,

    id as source_report_order_id,
