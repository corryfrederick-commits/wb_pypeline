{{ config(materialized='table', schema='quarantine', alias='rq_items_warehouses_quality', tags=['row_quality']) }}

with base as (
    select *
    from {{ ref('stg_items_warehouses') }}
),

issues as (
    select *
    from {{ ref('rq_items_warehouses_issues') }}
),
