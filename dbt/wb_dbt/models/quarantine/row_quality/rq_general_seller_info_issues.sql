{{ config(materialized='table', schema='quarantine', alias='rq_general_seller_info_issues', tags=['row_quality']) }}

with base as (
    select *
    from {{ ref('stg_general_seller_info') }}
),
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_general_seller_info__technical_raw_payload_id_missing'::text as rule_id,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_general_seller_info__technical_record_index_missing'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'required'::text as rule_type,
    'bad'::text as issue_severity,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_general_seller_info__technical_dataset_name_missing'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'required'::text as rule_type,
    'bad'::text as issue_severity,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_general_seller_info__technical_raw_record_missing'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'required'::text as rule_type,
    'bad'::text as issue_severity,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_general_seller_info__duplicate_raw_payload_record_index'::text as rule_id,
    'technical_lineage'::text as rule_group,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_general_seller_info__unexpected_dataset_name'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'accepted_dataset'::text as rule_type,
    'bad'::text as issue_severity,
    'dataset_name'::text as column_name,
    'unexpected_dataset_name'::text as issue_code,
    'dataset_name is not `general_seller_info`.'::text as issue_message,
    current_timestamp as detected_at
from (
    select
        base.*,
        (dataset_name <> 'general_seller_info') as is_issue
    from base
) q
where q.is_issue is true
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_general_seller_info__seller_identity_missing'::text as rule_id,
    'domain_required'::text as rule_group,
    'at_least_one_required'::text as rule_type,
    'warning'::text as issue_severity,
    'name'::text as column_name,
    'seller_identity_missing'::text as issue_code,
    'Seller info row has no seller identity-like field.'::text as issue_message,
    current_timestamp as detected_at
from (
    select
        base.*,
        ((name is null or nullif(name::text, '') is null)) as is_issue
    from base
) q
where q.is_issue is true
)
