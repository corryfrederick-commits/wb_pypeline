{{ config(materialized='table', schema='quarantine', alias='rq_communications_feedbacks_issues', tags=['row_quality']) }}

with base as (
    select *
    from {{ ref('stg_communications_feedbacks') }}
),
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_communications_feedbacks__technical_raw_payload_id_missing'::text as rule_id,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_communications_feedbacks__technical_record_index_missing'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'required'::text as rule_type,
    'bad'::text as issue_severity,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_communications_feedbacks__technical_dataset_name_missing'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'required'::text as rule_type,
    'bad'::text as issue_severity,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_communications_feedbacks__technical_raw_record_missing'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'required'::text as rule_type,
    'bad'::text as issue_severity,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_communications_feedbacks__duplicate_raw_payload_record_index'::text as rule_id,
    'technical_lineage'::text as rule_group,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_communications_feedbacks__unexpected_dataset_name'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'accepted_dataset'::text as rule_type,
    'bad'::text as issue_severity,
    'dataset_name'::text as column_name,
    'unexpected_dataset_name'::text as issue_code,
    'dataset_name is not `communications_feedbacks`.'::text as issue_message,
    current_timestamp as detected_at
from (
    select
        base.*,
        (dataset_name <> 'communications_feedbacks') as is_issue
    from base
) q
where q.is_issue is true
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_communications_feedbacks__feedback_identity_missing'::text as rule_id,
    'domain_required'::text as rule_group,
    'at_least_one_required'::text as rule_type,
    'bad'::text as issue_severity,
    'id'::text as column_name,
    'feedback_identity_missing'::text as issue_code,
    'Feedback row has no feedback identity.'::text as issue_message,
    current_timestamp as detected_at
from (
    select
        base.*,
        ((id is null or nullif(id::text, '') is null)) as is_issue
    from base
) q
where q.is_issue is true
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_communications_feedbacks__feedback_date_missing'::text as rule_id,
    'domain_required'::text as rule_group,
    'at_least_one_required'::text as rule_type,
    'warning'::text as issue_severity,
    'created_date'::text as column_name,
    'feedback_date_missing'::text as issue_code,
    'Feedback row has no created/date field.'::text as issue_message,
    current_timestamp as detected_at
from (
    select
        base.*,
        ((created_date is null)) as is_issue
    from base
) q
where q.is_issue is true
)
