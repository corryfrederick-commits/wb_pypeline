{{ config(materialized='table', schema='quarantine', alias='rq_analytics_stocks_quality', tags=['row_quality']) }}

with base as (
    select *
    from {{ ref('stg_analytics_stocks') }}
),

issues as (
    select *
    from {{ ref('rq_analytics_stocks_issues') }}
),
