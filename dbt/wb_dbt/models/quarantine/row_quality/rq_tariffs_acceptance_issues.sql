{{ config(materialized='table', schema='quarantine', alias='rq_tariffs_acceptance_issues', tags=['row_quality']) }}

with base as (
    select *
    from {{ ref('stg_tariffs_acceptance') }}
),
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_tariffs_acceptance__technical_raw_payload_id_missing'::text as rule_id,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_tariffs_acceptance__technical_record_index_missing'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'required'::text as rule_type,
    'bad'::text as issue_severity,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_tariffs_acceptance__technical_dataset_name_missing'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'required'::text as rule_type,
    'bad'::text as issue_severity,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_tariffs_acceptance__technical_raw_record_missing'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'required'::text as rule_type,
    'bad'::text as issue_severity,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_tariffs_acceptance__duplicate_raw_payload_record_index'::text as rule_id,
    'technical_lineage'::text as rule_group,
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_tariffs_acceptance__unexpected_dataset_name'::text as rule_id,
    'technical_lineage'::text as rule_group,
    'accepted_dataset'::text as rule_type,
    'bad'::text as issue_severity,
    'dataset_name'::text as column_name,
    'unexpected_dataset_name'::text as issue_code,
    'dataset_name is not `tariffs_acceptance`.'::text as issue_message,
    current_timestamp as detected_at
from (
    select
        base.*,
        (dataset_name <> 'tariffs_acceptance') as is_issue
    from base
) q
where q.is_issue is true
...
select
    ...
    q.record_index,
    q.dataset_name,
    'stg_tariffs_acceptance__tariff_context_missing'::text as rule_id,
    'domain_required'::text as rule_group,
    'at_least_one_required'::text as rule_type,
    'warning'::text as issue_severity,
    'warehouse_name,warehouse_id'::text as column_name,
    'tariff_context_missing'::text as issue_code,
    'Tariff row has no warehouse/category/context field.'::text as issue_message,
    current_timestamp as detected_at
from (
    select
        base.*,
        ((warehouse_name is null or nullif(warehouse_name::text, '') is null) and (warehouse_id is null)) as is_issue
    from base
) q
where q.is_issue is true
)
