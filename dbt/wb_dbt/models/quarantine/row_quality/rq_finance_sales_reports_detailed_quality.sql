{{ config(materialized='table', schema='quarantine', alias='rq_finance_sales_reports_detailed_quality', tags=['row_quality']) }}
...
issues as (
    select *
    from {{ ref('rq_finance_sales_reports_detailed_issues') }}
),
